...
simulateWFM <- function(sel_cof, dom_par, mig_rat, pop_siz, ext_frq, int_frq, int_gen, lst_gen) {
  ...
  frq_pth <- simulateWFM_arma(fts_mat, mig_rat, pop_siz, ext_frq, int_frq, int_gen, lst_gen)
...
#' Standard version
runPMMH_1L <- function(sel_cof, dom_par, mig_rat, pop_siz, sel_gen, mig_gen, ext_frq, smp_gen, smp_siz, smp_cnt, ptn_num, pcl_num, itn_num) {
  if (any(is.na(smp_cnt[2, ]))) {
    smp_cnt[2, which(is.na(smp_cnt[2, ]))] <- -1
  }

  # run the PMMH
  PMMH <- runPMMH_1L_arma(sel_cof, dom_par, mig_rat, pop_siz, sel_gen, mig_gen, ext_frq, smp_gen, smp_siz, smp_cnt, ptn_num, pcl_num, itn_num)
...
#' Compiled version
cmprunPMMH_1L <- cmpfun(runPMMH_1L)
...
#' @param mig_rat the migration rate
#' @param pop_siz the number of the diploid individuals in the population
#' @param sel_gen the starting times of natural selection
#' @param mig_gen the starting time of gene migration
#' @param ext_frq the mutant allele frequencies (of the continent population)
#' @param smp_gen_A the sampling time points measured in one generation for TSHR
#' @param smp_siz_A the count of the chromosomes drawn from the population at all sampling time points for TSHR
#' @param smp_cnt_A the count of the mutant alleles and continent alleles observed in the sample at all sampling time points for TSHR
#' @param smp_gen_B the sampling time points measured in one generation for BCDO2
#' @param smp_siz_B the count of the chromosomes drawn from the population at all sampling time points for BCDO2
#' @param smp_cnt_B the count of the mutant alleles and continent alleles observed in the sample at all sampling time points for BCDO2
#' @param ptn_num the number of subintervals divided per generation in the Euler-Maruyama method
#' @param pcl_num the number of particles generated in the bootstrap particle filter
#' @param itn_num the number of the iterations carried out in the PMMH

#' Standard version
runPMMH_2L <- function(sel_cof, dom_par, mig_rat, pop_siz, sel_gen, mig_gen, ext_frq, smp_gen_A, smp_siz_A, smp_cnt_A, smp_gen_B, smp_siz_B, smp_cnt_B, ptn_num, pcl_num, itn_num) {
  if (any(is.na(smp_cnt_A[2, ]))) {
    smp_cnt_A[2, which(is.na(smp_cnt_A[2, ]))] <- -1
  }
  if (any(is.na(smp_cnt_B[2, ]))) {
    smp_cnt_B[2, which(is.na(smp_cnt_B[2, ]))] <- -1
  }

  # run the PMMH
  PMMH <- runPMMH_2L_arma(sel_cof, dom_par, mig_rat, pop_siz, sel_gen, mig_gen, ext_frq, smp_gen_A, smp_siz_A, smp_cnt_A, smp_gen_B, smp_siz_B, smp_cnt_B, ptn_num, pcl_num, itn_num)

  return(list(sel_cof_A_chn = as.vector(PMMH$sel_cof_A_chn),
              sel_gen_A_chn = as.vector(PMMH$sel_gen_A_chn),
              frq_pth_A_chn = as.array(PMMH$frq_pth_A_chn)[, (0:(max(smp_gen_A) - min(smp_gen_A))) * ptn_num + 1, ],
              sel_cof_B_chn = as.vector(PMMH$sel_cof_B_chn),
              sel_gen_B_chn = as.vector(PMMH$sel_gen_B_chn),
              frq_pth_B_chn = as.array(PMMH$frq_pth_B_chn)[, (0:(max(smp_gen_B) - min(smp_gen_B))) * ptn_num + 1, ],
              mig_rat_chn = as.vector(PMMH$mig_rat_chn)))
}
#' Compiled version
cmprunPMMH_2L <- cmpfun(runPMMH_2L)
