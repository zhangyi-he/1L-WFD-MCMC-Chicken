...
// [[Rcpp::export]]
arma::dmat simulateWFM_arma(const arma::dmat& fts_mat, const double& mig_rat, const int& pop_siz, const double& ext_frq, const arma::dcolvec& int_frq, const int& int_gen, const int& lst_gen) {
  // ensure RNG gets set/reset
  RNGScope scope;
...
List runPMMH_1L_arma(const double& sel_cof, const double& dom_par, const double& mig_rat, const int& pop_siz, const int& sel_gen, const int& mig_gen, const double& ext_frq, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::imat& smp_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num, const arma::uword& itn_num) {
  ...
  arma::dcube frq_pth = arma::zeros<arma::dcube>(4, arma::uword(arma::max(smp_gen) - arma::min(smp_gen)) * ptn_num + 1, pcl_num);
  calculateLogLikelihood_arma(log_lik(0), frq_pth, sel_cof_chn(0), dom_par, mig_rat_chn(0), pop_siz, sel_gen_chn(0), mig_gen, ext_frq, smp_gen, smp_siz, ptl_cnt, ptn_num, pcl_num);
  ...
  for (arma::uword i = 1; i < itn_num; i++) {
    ...
    calculateLogLikelihood_arma(log_lik(1), frq_pth, sel_cof_chn(i), dom_par, mig_rat_chn(i - 1), pop_siz, sel_gen_chn(i), mig_gen, ext_frq, smp_gen, smp_siz, ptl_cnt, ptn_num, pcl_num);
    ...
    // calculate the likelihood
    calculateLogLikelihood_arma(log_lik(0), frq_pth, sel_cof_chn(i), dom_par, mig_rat_chn(i), pop_siz, sel_gen_chn(i), mig_gen, ext_frq, smp_gen, smp_siz, ptl_cnt, ptn_num, pcl_num);
...
// Update the selection-related parameters in the blockwise particle marginal Metropolis-Hastings
// [[Rcpp::export]]
void updateSelection_arma(double& log_lik, arma::dmat& frq_pth, double& sel_cof_new, int& sel_gen_new, const double& sel_cof, const double& dom_par, const double& mig_rat, const int& pop_siz, const int& sel_gen, const int& mig_gen, const double& ext_frq, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::field<arma::imat>& ptl_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num, const double& sel_cof_sd, const double& sel_gen_sd) {
  // ensure RNG gets set/reset
  RNGScope scope;

  // generate the selection-related parameters from the adaptive truncated normal distribution
  do {
    sel_cof_new = sel_cof + sel_cof_sd * arma::randn();
  }
  while (sel_cof_new > 1);
  do {
    sel_gen_new = sel_gen + int(round(sel_gen_sd * arma::randn()));
  }
  while (sel_gen_new > smp_gen.max());
  // while (sel_gen_new < smp_gen.min() || sel_gen_new > smp_gen.max());

  // calculate the likelihood
  double log_lik_new = 0;
  arma::dcube frq_pth_new = arma::zeros<arma::dcube>(4, arma::uword(arma::max(smp_gen) - arma::min(smp_gen)) * ptn_num + 1, pcl_num);
  calculateLogLikelihood_arma(log_lik_new, frq_pth_new, sel_cof_new, dom_par, mig_rat, pop_siz, sel_gen_new, mig_gen, ext_frq, smp_gen, smp_siz, ptl_cnt, ptn_num, pcl_num);

  // calculate the proposal
  double log_psl = -log(arma::normcdf(1.0, sel_cof_new, sel_cof_sd)) -
    log(arma::normcdf(double(smp_gen.max()), double(sel_gen_new), sel_gen_sd));
  // log(arma::normcdf(double(smp_gen.max()), double(sel_gen_new), sel_gen_sd) - arma::normcdf(double(smp_gen.min()), double(sel_gen_new), sel_gen_sd));
  double log_psl_new = -log(arma::normcdf(1.0, sel_cof, sel_cof_sd)) -
    log(arma::normcdf(double(smp_gen.max()), double(sel_gen), sel_gen_sd));
  // log(arma::normcdf(double(smp_gen.max()), double(sel_gen), sel_gen_sd) - arma::normcdf(double(smp_gen.min()), double(sel_gen), sel_gen_sd));

  // calculate the acceptance ratio
  double alpha = log_lik_new == -arma::datum::R_inf ? 0 : exp((log_lik_new - log_psl_new) - (log_lik - log_psl));
  alpha = (alpha > 1) ? 1 : alpha;
  if (arma::randu() > alpha) {
    sel_cof_new = sel_cof;
    sel_gen_new = sel_gen;
...
List runPMMH_2L_arma(const arma::dcolvec& sel_cof, const arma::dcolvec& dom_par, double& mig_rat, const int& pop_siz, const arma::icolvec& sel_gen, const int& mig_gen, const arma::dcolvec& ext_frq, const arma::irowvec& smp_gen_A, const arma::irowvec& smp_siz_A, const arma::imat& smp_cnt_A, const arma::irowvec& smp_gen_B, const arma::irowvec& smp_siz_B, const arma::imat& smp_cnt_B, const arma::uword& ptn_num, const arma::uword& pcl_num, const arma::uword& itn_num) {
  ...
  arma::field<arma::imat> ptl_cnt_A(smp_gen_A.n_elem);
  for (arma::uword k = 0; k < smp_gen_A.n_elem; k++) {
    ptl_cnt_A(k) = calculateAlleleCnt_arma(smp_siz_A(k), smp_cnt_A.col(k));
  }

  arma::field<arma::imat> ptl_cnt_B(smp_gen_B.n_elem);
  for (arma::uword k = 0; k < smp_gen_B.n_elem; k++) {
    ptl_cnt_B(k) = calculateAlleleCnt_arma(smp_siz_B(k), smp_cnt_B.col(k));
  }

  arma::drowvec sel_cof_A_chn = arma::zeros<arma::drowvec>(itn_num);
  arma::irowvec sel_gen_A_chn = arma::zeros<arma::irowvec>(itn_num);
  arma::dcube frq_pth_A_chn = arma::zeros<arma::dcube>(4, arma::uword(max(smp_gen_A) - min(smp_gen_A)) * ptn_num + 1, itn_num);

  arma::drowvec sel_cof_B_chn = arma::zeros<arma::drowvec>(itn_num);
  arma::irowvec sel_gen_B_chn = arma::zeros<arma::irowvec>(itn_num);
  arma::dcube frq_pth_B_chn = arma::zeros<arma::dcube>(4, arma::uword(max(smp_gen_B) - min(smp_gen_B)) * ptn_num + 1, itn_num);

  arma::drowvec mig_rat_chn = arma::zeros<arma::drowvec>(itn_num);
  // arma::irowvec mig_gen_chn = arma::zeros<arma::irowvec>(itn_num);

  // arma::drowvec log_pri_A = arma::zeros<arma::drowvec>(2);
  // arma::drowvec log_lik_A = arma::zeros<arma::drowvec>(2);
  // arma::drowvec log_psl_A = arma::zeros<arma::drowvec>(2);
  double log_lik_A = 0;

  // arma::drowvec log_pri_B = arma::zeros<arma::drowvec>(2);
  // arma::drowvec log_lik_B = arma::zeros<arma::drowvec>(2);
  // arma::drowvec log_psl_B = arma::zeros<arma::drowvec>(2);
  double log_lik_B = 0;
  ...
  arma::dcolvec sel_cof_sd = {5e-03, 1e-02};
  arma::dcolvec sel_gen_sd = {2e+01, 5e+01};
  double mig_rat_sd = 1e-03;
  // double mig_gen_sd = 1e+01;
  ...
  // take the uniform prior and fix the initial value of the population genetic parameters to zero
  // or take the beta prior with alpha = 1 and beta = 3
  sel_cof_A_chn(0) = sel_cof(0);
  sel_gen_A_chn(0) = sel_gen(0);

  sel_cof_B_chn(0) = sel_cof(1);
  sel_gen_B_chn(0) = sel_gen(1);

  mig_rat_chn(0) = mig_rat;
  // mig_gen_chn(0) = mig_gen;

  arma::dcube frq_pth_A = arma::zeros<arma::dcube>(4, arma::uword(arma::max(smp_gen_A) - arma::min(smp_gen_A)) * ptn_num + 1, pcl_num);
  calculateLogLikelihood_arma(log_lik_A, frq_pth_A, sel_cof_A_chn(0), dom_par(0), mig_rat_chn(0), pop_siz, sel_gen_A_chn(0), mig_gen, ext_frq(0), smp_gen_A, smp_siz_A, ptl_cnt_A, ptn_num, pcl_num);
  frq_pth_A_chn.slice(0) = frq_pth_A.slice(0);

  arma::dcube frq_pth_B = arma::zeros<arma::dcube>(4, arma::uword(arma::max(smp_gen_B) - arma::min(smp_gen_B)) * ptn_num + 1, pcl_num);
  calculateLogLikelihood_arma(log_lik_B, frq_pth_B, sel_cof_B_chn(0), dom_par(1), mig_rat_chn(0), pop_siz, sel_gen_B_chn(0), mig_gen, ext_frq(1), smp_gen_B, smp_siz_B, ptl_cnt_B, ptn_num, pcl_num);
  frq_pth_B_chn.slice(0) = frq_pth_B.slice(0);

  double alpha = 0;
  for (arma::uword i = 1; i < itn_num; i++) {
    cout << "iteration: " << i + 1 << endl;

    // generate the selection-related parameters at locus A from the adaptive truncated normal distribution
    double sel_cof_A = 0;
    int sel_gen_A = 0;
    ...
    // calculate the proposal
    log_psl(0) = -log(arma::normcdf(1.0, mig_rat_chn(i), mig_rat_sd) - arma::normcdf(0.0, mig_rat_chn(i), mig_rat_sd));
    // log(arma::normcdf(double(smp_gen(arma::find(smp_cnt.row(1) > 0).min())), double(mig_gen_chn(i)), mig_gen_sd));
    // log(arma::normcdf(double(smp_gen(arma::find(smp_cnt.row(1) > 0).min())), double(mig_gen_chn(i)), mig_gen_sd) - arma::normcdf(double(smp_gen.min()), double(mig_gen_chn(i)), mig_gen_sd));
    log_psl(1) = -log(arma::normcdf(1.0, mig_rat_chn(i - 1), mig_rat_sd) - arma::normcdf(0.0, mig_rat_chn(i - 1), mig_rat_sd));
    // log(arma::normcdf(double(smp_gen(arma::find(smp_cnt.row(1) > 0).min())), double(mig_gen_chn(i - 1)), mig_gen_sd));
    // log(arma::normcdf(double(smp_gen(arma::find(smp_cnt.row(1) > 0).min())), double(mig_gen_chn(i - 1)), mig_gen_sd) - arma::normcdf(double(smp_gen.min()), double(mig_gen_chn(i - 1)), mig_gen_sd));
  ...
  return List::create(Named("sel_cof_A_chn", sel_cof_A_chn),
                      Named("sel_gen_A_chn", sel_gen_A_chn),
                      Named("frq_pth_A_chn", frq_pth_A_chn),
                      Named("sel_cof_B_chn", sel_cof_B_chn),
                      Named("sel_gen_B_chn", sel_gen_B_chn),
                      Named("frq_pth_B_chn", frq_pth_B_chn),
                      // Named("mig_gen_chn", mig_gen_chn),
                      Named("mig_rat_chn", mig_rat_chn));
}
/*************************/
